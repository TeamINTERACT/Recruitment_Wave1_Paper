...
### Same linear model using the lm function.. 
...
model_testing <- function(y, Xs, data, family){
  ...
  # Run model
  glm(formula_express, data = data,  family = family)
}
...
xlist <- list (Reach= c("lg_Reach", "lg_R_Lag2", "lg_R_Lag3", "lg_R_Lag4", "lg_R_Lag5" ),
               ...
               Letter= c( "Letter_Lag5", "Letter_Lag10", "Letter_Lag15","Letter_Lag20" ),
...
combX2<-  list("lg_R_Lag2" ,"SM_Lag2","Letter_Lag10","Letter_Reminder_Lag5",
               "Partners_Lag2","Other_Lag2","Snowball_Lag4","Other_Media_Lag2",
               "Big_newspaper_Lag2")


combX3<-  list("lg_R_Lag2" ,"SM_Lag2","Letter_Lag15","Letter_Reminder_Lag5",
               "Partners_Lag2","Other_Lag2","Snowball_Lag4","Other_Media_Lag2",
               "Big_newspaper_Lag2")



combX4<-  list("lg_R_Lag2" ,"SM_Lag2","Letter_Lag15_M","Letter_Reminder_Lag2",
               "Partners_Lag10","Other_Lag2","Snowball_Lag4", "Other_Media_Lag2",
               "Big_newspaper_Lag2")

Methods_AND_Recruited$Letter_Lag10_M <- Methods_AND_Recruited[, "Letter_Lag10"]

sum(Methods_AND_Recruited$Letter_Lag10_M)

Methods_AND_Recruited[Methods_AND_Recruited$Letter_Lag10_M!=0, "Letter_Lag10_M"] <- 0.1

Methods_AND_Recruited$Letter_Lag15_M <- Methods_AND_Recruited[, "Letter_Lag15"]

sum(Methods_AND_Recruited$Letter_Lag15_M)

Methods_AND_Recruited[Methods_AND_Recruited$Letter_Lag15_M!=0, "Letter_Lag15_M"] <- 0.066667


# Test different lms
model_best_combX4 <- model_testing3(y = "freq", combX4, data = Methods_AND_Recruited, family = gaussian())
summary(model_best_combX4)




# Test different lms
model_best_combX3 <- model_testing3(y = "freq", combX3, data = Methods_AND_Recruited, family = gaussian())
summary(model_best_combX3)



tab_model(model_best_combX3,  p.style = "asterisk")
...
model_best_combX_linear <- model_testing3(y = "freq", combX2, data = Methods_AND_Recruited)
summary(model_best_combX_linear)



model_combine <- model_testing(y = "freq", c(combX, x_main_exposure), 
                               data = Methods_AND_Recruited, family = gaussian())
summary(model_combine)



tab_model(model_best_combX4,  p.style = "asterisk")


table1:: table1 (~  Reach + Social_Media_exposure + Letter_exposure + 
Partners_exposure +  Other_exposure +  
Snowball_Campaign_exposure +  Other_Media_exposure +
Big_newspapers_exposure + Letter_Reminder_exposure , data = Methods_AND_Recruited, overall="Total", topclass="Rtable1-zebra")


model_best_comb1 <- model_testing2(y = "freq" ,  comb2 , data = Methods_AND_Recruited, family = gaussian())
summary(model_best_comb1)
glance(model_best_comb1)



tab_model(model_best_comb1, model_best_comb12,  p.style = "asterisk")




summary(Methods_AND_Recruited[!Methods_AND_Recruited$Reach %in%  0, "Reach"])





# Trying negative binomial glm didnot work ... 
...
model_testing3 <- function(y, Xs, data, family){
  ...
  model <- glm.nb(formula_express, data = data,  start = NULL,
                  method = "glm.fit",
                  model = TRUE, x = FALSE, y = TRUE, contrasts = NULL,
                  link = identity)
}
...
model_test <- model_testing3(y = "freq",  comb2 , data = Methods_AND_Recruited_with_lg)
